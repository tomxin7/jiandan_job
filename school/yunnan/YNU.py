
import tomxin.getInfo

#def getYNU():
#云南大学
if __name__ == '__main__':
    url = "http://jobs.ynu.edu.cn/wszplist.jsp?urltype=tree.TreeTempUrl&wbtreeid=1091"#高校就业网的网址
    html = tomxin.getInfo.get_source(url,"utf-8")
    info = tomxin.getInfo.get_info(html,'alternatecolor">','</table>')
    title = tomxin.getInfo.get_content(info,'href=".+?>','<')
    url = tomxin.getInfo.get_content(info,'href="','"')
    i=0
    for u in url[:2]:
        r_city="云南"
        r_school="云南大学"
        r_title=title[i]
        r_trait = "YNU" + u[-117:]#这里要自己写提取规则
        r_url = "http://jobs.ynu.edu.cn/" + u
        print(r_title)
        print(r_trait)
        print(r_url)
        i += 1
    #     r_content = tomxin.getInfo.get_url_content(r_url, "utf-8", 'strat', 'end')
    #     print(r_content)#这里获得详情页的内容
    #     print(r_title + "\n" + r_url)

